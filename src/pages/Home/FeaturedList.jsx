...
const FeaturedList = () => {
  const navigate = useNavigate();
    ...
    const featuredList = [
      ...
      const lists = featuredList.slice(0, 10)
    ...
    <div className='overflow-x-scroll overflow-y-hidden dmovies grid grid-rows-[50vh] grid-cols-auto whitespace-wrap w-full relative h-full max-w-screen-xl mx-auto'>
      <div className="flex relative h-max">
        {lists.map((list, index) => (
          <div key={index} className='w-[500px] relative cursor-pointer bg-[#151516] movie mr-4 rounded-2xl text-center' 
          onClick={() => {
            navigate(`/list/${encodeURIComponent(list.id)}`);
          
        }}>
            <div>
            <img className='w-full h-full object-contain rounded-3xl' src={list.backdropPath} alt={list.name} />
            <div className='absolute bottom-0 left-0 right-0'>
                <span className='text-white block py-4 rounded-b-2xl font-sans font-semibold' style={{ backdropFilter: 'blur(10px)' }}>{list.name}</span>
                </div>
            </div>
          </div>
        ))}
