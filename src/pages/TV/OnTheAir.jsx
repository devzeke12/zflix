...
const OnTheAir = () => {
  ...
  return (
    <div className='w-full h-screen flex flex-col gap-10' >
        <div>
            <h1 className='text-3xl font-bold'>on The Air</h1>
            <button className='bg-[#33333A] px-3 py-1 rounded-3xl my-3' onClick={() => navigate('/tvshows/on-the-air/1')}>View More</button>
        </div>
      ...
      <div className="flex h-max">
        {onAir.map((tvshow, index) => <TvCard key={index} poster_path={tvshow.poster_path} title={tvshow.title} original_name={tvshow.original_name} id={tvshow.id} />)}
      </div>
    </div>
    </div>
  );
