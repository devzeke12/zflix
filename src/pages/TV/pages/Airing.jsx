...
const Airing = () => {
  ...
  const getTvs = (pageNumber) => {
    setLoading(true);
    fetch(`https://api.themoviedb.org/3/tv/airing_today?language=en-US&page=${pageNumber}&api_key=${API_KEY}`)
  ...
  const handlePageClick = (newPage) => {
    navigate(`/tvshows/airing/${encodeURIComponent(newPage)}`);
    ...
    <div>
      ...
      <div className='flex flex-row flex-wrap justify-center gap-5'>
        {loading ? (
          ...
          tvs.map((movie, index) => layout === 'card' ? (
