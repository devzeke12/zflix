...
const NowPlaying = () => {
  ...
  const getMovies = (pageNumber) => {
    setLoading(true);
    fetch(`https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=${pageNumber}&api_key=${API_KEY}`)
  ...
  const handlePageClick = (newPage) => {
    navigate(`/movies/now-playing/${encodeURIComponent(newPage)}`);
    ...
    <div>
      ...
      <div className='flex flex-row flex-wrap justify-center gap-5'>
        {loading ? (
          ...
          movies.map((movie, index) => layout === 'card' ? (
